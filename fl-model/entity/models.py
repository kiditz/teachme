...
t_learning_group_user = db.Table(
	...
	db.Column('group_id', db.ForeignKey(u'tm_learning_group.group_id'), nullable=False, primary_key=True),
	db.Column('user_id', db.ForeignKey(u'tm_user.user_id'), nullable=False, primary_key=True)
)
...
class UserPrincipal(db.Model, Entity):
	...
	document_id = db.Column(db.ForeignKey(u'tm_document.document_id'), index=True)
...
class UserAuthority(db.Model, Entity):
	...
	id = db.Column('user_authority_id', db.BigInteger, db.Sequence('tm_user_authority_user_authority_id_seq'),
	               primary_key=True)
...
class SchoolLevel(db.Model, Entity):
	...
	id = db.Column('school_level_id', db.BigInteger, db.Sequence('tm_school_level_school_level_id_seq'),
	               primary_key=True)
...
class SchoolClass(db.Model, Entity):
	...
	id = db.Column('school_class_id', db.BigInteger, db.Sequence('tm_school_class_school_class_id_seq'),
	               primary_key=True)
	name = db.Column(db.String(30), nullable=False, index=True)
	level_id = db.Column(db.ForeignKey(u'tm_school_level.school_level_id'), index=True)
	created_at = db.Column(db.DateTime(timezone=False), default=datetime.now)
...
class TaskQuestion(db.Model, Entity):
	...
	id = db.Column('question_id', db.BigInteger, db.Sequence('tm_task_question_question_id_seq'), primary_key=True)
...
class TaskAnswer(db.Model, Entity):
	...
	id = db.Column('answer_id', db.BigInteger, db.Sequence('tm_task_answer_answer_id_seq'), primary_key=True)
	question_id = db.Column(db.ForeignKey(u'tm_task_question.question_id'), nullable=False)
...
class LearningGroup(db.Model, Entity):
	...
	id = db.Column('group_id', db.BigInteger, db.Sequence('tm_learning_group_group_id_seq'), primary_key=True)
...
class TaskGroup(db.Model, Entity):
	...
	task_id = db.Column(db.ForeignKey(u'tm_task.task_id'), nullable=False, primary_key=True)
	group_id = db.Column(db.ForeignKey(u'tm_learning_group.group_id'), nullable=False, primary_key=True)
	created_at = db.Column(db.DateTime(timezone=False), default=datetime.now)
	update_at = db.Column(db.DateTime(timezone=False), onupdate=datetime.now)
...
if __name__ == '__main__':
	app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('db.name',
	                                                       'postgresql://kiditz:rioters7@172.17.0.1:2070/teachme')
