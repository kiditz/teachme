...
class TaskScoreConsumer(multiprocessing.Process):
	...
	def __init__(self):
		...
		self.consumer = KafkaConsumer(group_id='task-group', bootstrap_servers=[app.config['KAFKA_SERVER']])
		...
		while not self.stop_event.is_set():
			...
			for msg in self.consumer:
				...
				for it in answers:
					...
					total_mc = TaskQuestion.query.filter_by(id = it['question_id']).filter_by(question_type='MC').count()
					total_non_mc = TaskQuestion.query.filter_by(id = it['question_id']).filter(TaskQuestion.question_type != 'MC').count()
					total_mc = float(total_mc)					
					score_mc = 0.0
					if question.question_type == 'MC':
						answer_key = question.answer_key
						user_answer = it['answer']
						answer_input = {'question_id' : it['question_id'], 'user_id': task['user_id']}
				...
				try:
					db.session.bulk_insert_mappings(QuestionScore, answers_list)					
					db.session.commit()
